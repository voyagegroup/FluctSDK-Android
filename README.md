...
* 動画リワード/動画インタースティシャル広告の表示時にFireOSでクラッシュする問題を修正

## 2020/8/18

### v8.6.2

* RTBによる動画広告表示において、一部接続先広告事業者の提供するデータに不正なURLが含まれる場合にエンドカードが表示されない問題を修正
...
* MoPub v5.13.0未満向けBidLift動画インタースティシャルアダプターを作成
* MoPub v5.13.0未満向け動画インタースティシャルメディエーションアダプターを作成
* MoPub v5.13.0未満向けBidLifバナーアダプターを作成
* MoPub v5.13.0未満向けバナーメディエーションアダプターを作成
...
* MoPub向け動画リワード広告再生時、稀にクラッシュする問題を修正
...
* AdMob, MoPub向けBid Lift動画インタースティシャル広告表示機能を追加
* MoPub向け動画リワード広告で稀に再生失敗が発生する問題を修正
...
* MoPub SDKのv5.13.0でバナーのパスバックに問題があるため、最低サポートバージョンをv5.13.1に変更
...
* MoPub SDKのv5.13.0のMediation Adapterの新しいインターフェースに対応
* MoPub SDKの最低サポートバージョンをv5.13.0に変更

## 2020/6/10

### v7.9.0

* MoPub向けBid Lift動画リワード広告表示機能を追加
...
* 一部WebView実装のインストールされた環境でバナーが正常に動作しない問題の回避コードを追加
* 一部WebView実装のインストールされた環境で動画広告が正常に動作しない問題の回避コードを追加
...
* インタースティシャル広告用カスタムイベント内部でFluctSDKの動画インタースティシャル広告を利用

### MoPub向けカスタムイベント v7.6.0

* インタースティシャル広告用カスタムイベントの対応
